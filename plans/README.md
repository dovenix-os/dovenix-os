...
4. **Phases are human-verifiable and human-gated.** Every phase ends in
   ...
   the phase-completion protocol in `CLAUDE.md` (audit: reliability, Rust
   idioms, and performance first, security last → docs updated and confirmed →
   pause for review). Agents do not roll from one phase into the next
   unreviewed.
